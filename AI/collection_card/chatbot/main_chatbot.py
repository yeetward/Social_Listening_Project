...
def interactive_mode(company_name: str, stream: bool = True):
    ...
    while True:
        try:
            ...
            # Get response
            response = chatbot.chat(context, user_input, conversation_history, stream)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Interactive chatbot for business intelligence"
    )
    parser.add_argument("company_name", type=str, help="Company name")
    parser.add_argument(
        "--no-stream", action="store_true", help="Disable streaming effect"
    )
    args = parser.parse_args()

    interactive_mode(args.company_name, stream=not args.no_stream)
