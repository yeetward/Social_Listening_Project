...
def load_context(company_name: str, num_articles=20):
    ...
    articles = list(
        ...
        ).sort("created_at", -1).limit(num_articles)
    )
...
def chat(context: dict, user_message: str, conversation_history: list = None, stream: bool = True):
    """
...
"""
    ...
    try:
        ...
        response = gpt.load_model(prompt, max_tokens=1500, temperature=0.4, stream=False)
