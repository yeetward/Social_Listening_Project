...
from AI.collection_card.backlinks.fetch_backlinks import get_backlink_candidates_from_articles
from AI.collection_card.backlinks.analyse_backlinks import analyse_backlinks

# Optional, only if/when you want to persist into company_profiles.cards.backlinks
try:
    from AI.collection_card.company_cards import upsert_company_card
except Exception:
    upsert_company_card = None
...
def run(
    ...
    *,
    ...
    """
    Engagement-oriented backlinks (earned mentions) for a company:
      1) Pull AI-enriched articles that mention the company (via company_id)
      2) Normalize to backlink candidates with real source_url
      3) Score for 'engage now' (social + fresh + high-signal)
      4) (Optional) LLM label + optional safe persistence
    """
    try:
        log(" Fetching mention/backlink candidates…", verbose)
        candidates = get_backlink_candidates_from_articles(
            company_id,
            limit=max(limit, 50),          # fetch a bit more to score
            min_relevance=min_relevance,
        )
        if not candidates:
            log("No candidates found.", verbose)
            return []

        log(f" {len(candidates)} candidates", verbose)

        # We don't need the company name here because scoring is source/recency-based.
        # For LLM labels, pass a friendly name (optional: fetch from shared fetcher if you want).
        ...
        log(f" Returning {len(analysed)} backlinks", verbose)

        if persist and upsert_company_card:
            try:
                upsert_company_card(company_id, "backlinks", {"items": analysed})
                log("  Stored in company_profiles.cards.backlinks", verbose)
            except Exception as pe:
                # Swallow storage errors to avoid breaking the API path
                log(f"(storage skipped) {pe}", verbose)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Engagement-oriented backlinks for a company (company_id-first).")
    p.add_argument("company_id", type=str, help="ObjectId of the company in company_profiles")
    p.add_argument("--limit", type=int, default=20, help="Max items to return")
    p.add_argument("--min-relevance", type=float, default=0.30, help="Min relevance_score from ai_results")
    p.add_argument("--with-llm", action="store_true", help="Enable LLM sentiment/category/note")
    p.add_argument("--persist", action="store_true", help="Upsert into company_profiles.cards.backlinks")
    p.add_argument("--verbose", action="store_true", help="Debug logs to stderr")
    args = p.parse_args()
