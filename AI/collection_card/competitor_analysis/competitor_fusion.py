...
# #     text: str, *, industry: Optional[List[str]], location: Optional[List[str]]
# # ) -> float:
# #     """
...
# #         reasons = []
# #         if ss >= 0.5:
# #             reasons.append(f"co-mentioned with {seed_brand} in multiple sentences")
# #         elif ss > 0:
# #             reasons.append(f"co-mentioned with {seed_brand}")
# #         if cc > 0:
# #             reasons.append("appears near competitor language")
# #         if il > 0:
# #             reasons.append("matches industry/location context")
# #         if dh > 0:
...
# # AI/collection_card/competitor_fusion.py
# from __future__ import annotations
...
# # Try SBERT for tiny name-sim boost; safe if missing
# try:
...
# _COMP_TERMS = {
#     "competitor", "competitors", "rival", "rivals", "competing",
#     "competition", "versus", "vs", "alternative", "alternatives",
#     "compared to", "against"
# }
# _WORD = re.compile(r"[A-Za-z0-9]+")
...
# ) -> List[Dict[str, Any]]:
#     """
#     Main API: returns sorted competitors:
#     [{ name, score, signals:{...}, explanation }]
#     """
#     if not seed_brand or not (text or "").strip():
#         return []

#     # 1) candidates
#     candidates = extract_org_candidates(text)
#     if extra_candidates:
#         for c in extra_candidates:
#             if c and c not in candidates:
#                 candidates.append(c)

#     # Dedup and drop seed brand itself
#     seen = set()
#     uniq: List[str] = []
#     seed_low = seed_brand.lower().strip()
#     for c in candidates:
